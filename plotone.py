...
fig, axs = plt.subplots(2, 1, figsize=(80,40), sharex=True, sharey=True)
axs[0].plot(X_test[0,0:4000,:], color='salmon')
axs[0].set_title('ECG Signal 1')
axs[0].set_ylim([ymin, ymax])
axs[0].set_ylabel('signal value')
axs[0].get_yaxis().set_visible(False)

axs[1].plot(adv_sample_0_01[0:4000], color='salmon')
axs[1].set_title('ECG Signal 2')
axs[1].set_ylim([ymin, ymax])
axs[1].set_xlabel('index')
axs[1].set_ylabel('signal value')
...
axs.plot(X_test[0,0:4000,:], color='black', linewidth=2)
#axs.plot(adv_sample_0_1[0:4000], color='forestgreen', linewidth=2)
axs.set_xlabel('index')
axs.get_yaxis().set_visible(False)
axs.get_xaxis().set_visible(False)
axs.tick_params(labelsize=15)
axs.set_axis_off()
fig.savefig('test_4.png',bbox_inches='tight')