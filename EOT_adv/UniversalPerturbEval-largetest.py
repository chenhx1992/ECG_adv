...
pattern = r'LDMF_recfilter_w'+str(perturb_window)+'_e30_l2_A[0-9]+T'+str(target)+'.out'
        ...
        if re.match(pattern,inputstr) != None:
            ...
            for i, id_float in enumerate(target_id):
                ...
                for p in range(100):
                    ...
                    if p == 0:
                        test_all = zero_mean(filter(op_concate(perturb, perturb_window, pos)) + X_test_1)
                    else:
                        test_all = np.append(test_all, zero_mean(filter(op_concate(perturb, perturb_window, pos)) + X_test_1), axis=0)
