...
data = pd.read_csv('./Subjective_AMT/FinalStage/Batch_3413942_l2_l2diff_results.csv')

data1 = pd.read_csv('./Subjective_AMT/FinalStage/Batch_212040_batch_results_Part3-1.csv') 
data2 = pd.read_csv('./Subjective_AMT/FinalStage/Batch_212041_batch_results_Part3-2.csv') 
data3 = pd.read_csv('./Subjective_AMT/FinalStage/Batch_212042_batch_results_Part3-3.csv') 
data4 = pd.read_csv('./Subjective_AMT/FinalStage/Batch_212434_batch_results_Part3-4.csv')
...
# f1 = open("./Subjective_AMT/TODO_l2_l2diff_1.csv", "w")
# f2 = open("./Subjective_AMT/TODO_l2_l2diff_2.csv", "w")
# f3 = open("./Subjective_AMT/TODO_l2_l2diff_3.csv", "w")
# f4 = open("./Subjective_AMT/TODO_l2_l2diff_4.csv", "w")
# f5 = open("./Subjective_AMT/TODO_l2_l2diff_5.csv", "w")
# file_arr = [f1, f2, f3, f4, f5]
# col_count_arr = np.zeros(5)

# for index, rows in preference_question.iterrows():
#     if rows['l2'] + rows['l2diff'] < 5:
#         strs = []
...
#         todotimes = int(5-(rows['l2diff'] + rows['l2']))
...
print(l2diff_lwin, l2diff_swin, l2_lwin, l2_swin, numofpartial)
print(count, l2diff_lwin+l2diff_swin+l2_lwin+l2_swin+numofpartial)

ind = np.arange(600)
width = 0.5
p1 = plt.bar(ind, preference_question['l2'], width)
p2 = plt.bar(ind, preference_question['l2diff'], width, bottom=preference_question['l2'])
plt.legend((p1[0], p2[0]), ('L2-norm', 'Smooth+L2'), frameon=False)
...
p1 = plt.bar(ind, preference_question_a_n['l2diff'], width, color='sandybrown',  alpha=0.8)
...
plt.legend((p1[0], p2[0]), ('Smoothness+L2-norm', 'L2-norm'), frameon=False, bbox_to_anchor=(0., 1.0 , 1., .05), ncol=2, loc='upper center', fontsize=12)
...
for index, rows in preference_worker.iterrows():
    preference_worker.loc[index, 'l2diff_percent'] = rows['l2diff'] / (rows['l2'] + rows['l2diff']) * 100
    preference_worker.loc[index, 'l2_percent'] = rows['l2'] / (rows['l2'] + rows['l2diff']) * 100
...
fig, axs = plt.subplots(1, 1, figsize=(160,120))
width = 0.5
p1 = plt.bar(ind, preference_worker['l2diff_percent'], width, color='sandybrown',  alpha=0.8, hatch="\\")
...
print(l2diff_prefer, l2_prefer, tie)


df.to_csv('./paper_fig/ST_l2diff_l2_df.csv')
preference_question_groupbyclass.to_csv('./paper_fig/ST_l2diff_l2_preference_question_groupbyclass.csv')
preference_worker.to_csv('./paper_fig/ST_l2diff_l2_preference_worker.csv')
